#!/usr/bin/env python3
"""Roll up per-municipality green_areas_stats bands into one Parquet per region.

Reads the lakehouse catalog (latest ingest per municipality for dataset=areas),
loads each municipality areas-stats band from MinIO, appends ``ingest_at``, and writes:

  green_areas_admin_stats/region_id={id}/part-municipality-bands.parquet

Usage (host, MinIO on :9000):
  LAKEHOUSE_S3_ENDPOINT=http://localhost:9000 \\
    python rollup_admin_areas_stats.py
...
from areas_stats import areas_stats_hive_prefix  # noqa: E402
from gold_clusters import GOLD_BAND_MUNICIPALITY  # noqa: E402
from lakehouse_writer import (  # noqa: E402
    ...
    read_parquet_bytes,
    s3_client,
    write_admin_areas_region_municipality_bands,
)


def _latest_area_resolutions(catalog: pa.Table) -> list[dict]:
    """Max ingest_at per municipality for dataset=areas."""
    best: dict[int, dict] = {}
    cols = {name: catalog.column(name).to_pylist() for name in catalog.column_names}
    n = catalog.num_rows
    for i in range(n):
        if str(cols["dataset"][i]) != "areas":
            continue
...
def _read_municipality_areas_stats(client, resolution: dict) -> pa.Table | None:
...
def rollup(client) -> int:
    ...
    print(f"Rolling up {len(resolutions)} municipality areas-stats bands…")

    by_region: dict[int, list[pa.Table]] = defaultdict(list)
    missing = 0
    for res in resolutions:
        table = _read_municipality_areas_stats(client, res)
    ...
    for region_id, parts in sorted(by_region.items()):
        ...
        key = write_admin_areas_region_municipality_bands(
        ...
        written += 1
    print(f"Done. regions={written} missing_areas_stats={missing}")
    return written
