...
-- Where there are no surface (S) assets, lines and points can be placed; L and P must not intersect or overlap each other.
-- Implementation:
--   - Surfaces (S): disjoint grid cells per sub-area (central 60% of bbox; no overlap between S).
--   - Gap = sub-area minus (surface cells + tiny buffer). Only in the gap we place L and P (no S in the gap).
--   - Lines (L): segments inside the gap only (no overlap with S).
...
SELECT municipality_id, province_id, region_id, municipality_name, area_km2, geometry,
       GREATEST(20, LEAST(FLOOR(area_km2 * 3)::int, 300)) AS num_macro_areas, 5 AS sub_areas_per_macro,
       GREATEST(1000, LEAST(FLOOR(area_km2 * 250)::int, 120000)) AS num_trees,
       GREATEST(100, LEAST(FLOOR(area_km2 * 40)::int, 20000)) AS num_rows,
       GREATEST(50, LEAST(FLOOR(area_km2 * 25)::int, 12000)) AS num_lawns,
       GREATEST(50, LEAST(FLOOR(area_km2 * 30)::int, 15000)) AS num_hedges,
       GREATEST(25, LEAST(FLOOR(area_km2 * 15)::int, 8000)) AS num_flower_beds,
       GREATEST(50, LEAST(FLOOR(area_km2 * 20)::int, 10000)) AS num_street_greenery
...
-- STEP 4: Clean existing data and generate macro-areas (Voronoi = disjoint, no overlap; containment only)
\echo ''
...
-- level_id 1 = MANAGEMENT_UNIT (area_level); attribute_type_id 44 = ATT_25_000 Area fittizia (attribute_types)
...
-- STEP 5: Sub-areas (Voronoi). 5 sub_areas_per_macro => ~230k sub-areas (step 5 e 6 più veloci).
\echo ''
...
-- STEP 6: Green assets (P, L, S). No S in the gap; in the gap only L and P, with L and P not intersecting/overlapping.
-- Invariant: S in disjoint grid; gap = area minus S; L in gap; P in gap minus buffer(L).
-- geometry_type P (point), L (line), S (surface): docs/database/obt/types/attribute_types.md, 01-init-seed-04-attribute-types.sql.
-- Tuning: higher maintenance_work_mem for index build/update; parallel workers for scans; work_mem 1GB for whole step.
\echo ''
...
-- Grid of non-overlapping rectangles per sub-area for surface assets (S). Cap 6 cells (1 per type).
...
\echo '[6.1] Trees (geometry_type P; in gap, no overlap with surfaces or rows)...'
-- attribute_type_id 33 = ATT_03_108 Albero (P). Points from envelope then ST_Within(gap). Generate enough points so some fall in gap (gap often small vs envelope).
SET work_mem = '1GB';
...
\echo '[6.2] Rows (geometry_type L; parallel segments, no crossing; no overlap with trees/surfaces)...'
...
\echo '[6.3] Park (geometry_type S; grid cell 0, no overlap)...'
...
\echo '[6.3b] Urban forest (geometry_type S; grid cell 1, no overlap)...'
...
\echo '[6.4] Lawns (geometry_type S; grid cells 2+, no overlap)...'
...
\echo '[6.5] Hedges (geometry_type S; grid, no overlap)...'
-- attribute_type_id 28 = ATT_03_101 Cespuglio macchia/tappezzante (S)
INSERT INTO cadastre.green_assets (municipality_id, asset_type, geometry_type, geometry, green_area_id, region_id, province_id, attribute_type_id, species, attributes)
...
\echo '[6.6] Flower beds (geometry_type S; grid, no overlap)...'
...
\echo '[6.7] Street greenery (geometry_type S; grid, no overlap)...'
...
\echo ''
\echo '[STEP 7] Cleanup...'
DROP TABLE IF EXISTS _seed_municipalities_target, _seed_scaling, _seed_macro_areas, _seed_macro_ids, _seed_sub_areas, _seed_sotto_ids, _seed_sotto_gap, _seed_sotto_gap_trees, _seed_row_geoms, _seed_surface_grid;
