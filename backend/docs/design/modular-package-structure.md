...
| **application** | Use case (query/command); **usecases/query/cache/** contiene la logica di cache per i catalog (regions, provinces, municipalities, sub-municipal areas, green areas, green assets); i use case di catalog delegano a **query/cache/** per le letture in cache. | domain, infrastructure (repository concreti); i moduli in query/cache usano lazy import del repository per evitare dipendenze circolari. |
...
- **Cache nei use case di catalog:**  
  Sotto **territory/{geo,areas,assets}/application/usecases/query/** è presente la cartella **cache/** con moduli dedicati (es. `catalog_regions_cache.py`, `catalog_green_area_cache.py`). La logica di cache (LRU o TTL, chiavi, invalidazione) vive in questi moduli; i use case (es. `CatalogRegion`, `CatalogGreenArea`) delegano a funzioni tipo `get_cached_*` e re-esportano `invalidate_cache` per compatibilità. Vedi [folders-structure-be.md](./folders-structure-be.md) per l’albero completo.
