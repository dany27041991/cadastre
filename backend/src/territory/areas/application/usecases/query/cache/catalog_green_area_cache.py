...
def get_cached_green_areas(
    ...
    """Return green areas (from cache when applicable). Uses the dated lakehouse repo."""
    if contained_in_area_id is not None and municipality_id is not None:
        return repository.get_contained_or_intersecting_area(
    ...
    if sub_municipal_area_id is not None:
        ...
        result = repository.get_roots_by_municipality_intersecting_sub_municipal_area(
    ...
    if key in _green_area_cache:
        return _green_area_cache[key]
    result = repository.get_roots_by_municipality(municipality_id, region_id, province_id)
    _green_area_cache[key] = result
    return result
