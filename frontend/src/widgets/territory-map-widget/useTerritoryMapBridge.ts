...
export function useTerritoryMapBridge(map: UseGeoinsightMapBridgeResult): MapBridge {
  ...
  return useMemo(
    () => ({
      ...
      activateDrawPolygon: map.activateDrawPolygon,
      deactivateDrawGeometry: map.deactivateDrawGeometry,
      deleteAllDrawnGeometries: map.deleteAllDrawnGeometries,
      restrictSimpleDrawToClosedShapes: map.restrictSimpleDrawToClosedShapes,
      restoreSimpleDrawTools: map.restoreSimpleDrawTools,
      clearSimpleDrawGeometries: map.clearSimpleDrawGeometries,
      deactivateSimpleDrawTool: map.deactivateSimpleDrawTool,
      styleSimpleDrawOutline: map.styleSimpleDrawOutline,
      keepOnlyLastSimpleDrawGeometry: map.keepOnlyLastSimpleDrawGeometry,
      setKeepSimpleDrawClipFeatures: map.setKeepSimpleDrawClipFeatures,
      persistDrawClip: map.persistDrawClip,
      clearDrawClip: map.clearDrawClip,
      zoomToWgs84Bbox: map.zoomToWgs84Bbox,
    }),
    [map, leaf]
  )
}
