...
import { raiseGeoinsightSimpleDrawLayer } from '../geoinsightSimpleDrawTools'
import { reassertPersistedDrawClip } from './drawClipPersist'
  ...
  runGeoinsightOrQueue(host, () => {
    ...
    enqueueVendorOps(
      chunks.map((part, index) => () => {
        ...
        // Clip under the batch: restacking it after addGeometries put CL_draw
        // on top and stole cluster click-to-drill (vendor hit-tests topmost).
        if (index === 0) reassertPersistedDrawClip(ref, mapId)
